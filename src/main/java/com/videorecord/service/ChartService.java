...
public class ChartService {
	...
	public ResponseBean<ChartBean> getEchartsData(VideoSearchReqBean req) {
		...
		} else if (videoType.equals("电影") || videoType.equals("记录片")|| videoType.equals("网络视频")) {
			type = new String[] { "中国", "日本", "美国" };
			Set<String> c = new HashSet<String>();
			for (int i = 0; i < videoList.size(); i++) {
				if ((videoList.get(i).getVideoCountry() != "中国" || videoList.get(i).getVideoCountry() != "中国")
						&& videoList.get(i).getVideoCountry() != "日本" && videoList.get(i).getVideoCountry() != "美国") {
					c.add(videoList.get(i).getVideoCountry());
				}
			}
			for (String str : c) {
		...
		for (String season : type) {
			...
			for (int i = begin; i <= end; i++) {
				...
				for (int j = 0; j < videoList.size(); j++) {
					if (year.equals(videoList.get(j).getYear()) && season.equals(videoList.get(j).getVideoSeason())) {
