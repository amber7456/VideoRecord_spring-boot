...
public class ExcelDownloadController {
    ...
    public void getDiskList(HttpServletRequest request, HttpServletResponse response, VideoSearchReqBean req) throws Exception {
    ...
    // 生成excel文件
    public File createAdvEXCEL(VideoSearchReqBean req, VideoListBean videoList) {
        ...
        if (CommonUtils.isNotEmpty(req.getAddress()) && !req.getAddress().equals("ALL")) {
            nameStr.append("_" + diskMap.get(req.getAddress()));
        }
        //
        ...
        try {
            ...
            for (int i = 0; i < videoList.getYearData().size(); i++) {
                YearDataBean yearData = videoList.getYearData().get(i);
                int count = 0;
                int fuyu = 0;
                int haru = 0;
                int natsu = 0;
                ...
                for (int j = 0; j < yearData.getSeason().size(); j++) {
                    ...
                    for (int k = 0; k < videoInfo.size(); k++) {
                        ...
                        sheet.addCell(new Label(2, rowGroup, videoInfo.get(k).getVideoName(), wcsC));
                        String broadcast_time = CommonUtils.DateToString(videoInfo.get(k).getVideoBroadcastTime(),
                                "YYYY-MM-dd");
                        sheet.addCell(new Label(3, rowGroup, broadcast_time, wcsB));
                        sheet.addCell(new Label(4, rowGroup, videoInfo.get(k).getVideoEpisode() + "话", wcsB));
                        sheet.addCell(new Label(5, rowGroup, videoInfo.get(k).getVideoSource(), wcsB));
                        ...
                        for (int n = 0; n < videoInfo.get(k).getVideoResourceInfo().size(); n++) {
                            ...
                            if (videoResourceInfo.getResourceType().equals("BD")) {
                                wcf = wcf1;
                            } else {
                                wcf = wcf2;
                            }
                            ...
                            if (videoResourceInfo.getResourceType().equals("NORMAL")) {
                            ...
                            sheet.addCell(new Label(6 + r, rowGroup, videoResourceInfo.getResourceFormat(), wcf));
                            r++;

                            sheet.setColumnView(6 + r, 10);
                            sheet.addCell(new Label(6 + r, rowGroup, videoResourceInfo.getResourceResolution(), wcf));
                            r++;

                            sheet.setColumnView(6 + r, 6);
                            sheet.addCell(new Label(6 + r, rowGroup, videoResourceInfo.getResourceSubType(), wcf));
                            r++;

                            sheet.setColumnView(6 + r, 20);
                            sheet.addCell(new Label(6 + r, rowGroup,
                                    diskMap.get(videoResourceInfo.getResourceRecordAddress()), wcf));
                            r++;
    ...
    private HashMap<String, String> getDiskMap(List<DiskInfo> data) {
        ...
        for (int i = 0; i < data.size(); i++) {
            diskMap.put(data.get(i).getDiskId(), data.get(i).getDiskName());
        }
        return diskMap;
    }
